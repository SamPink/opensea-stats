...
def calc_current_ApeGang_median_price(last_n_sales=100):
    ...
    old = new = read_mongo(
        "ape-gang-old_sales", query_projection=mongo_projection, 
        query_sort=[('time',-1)],query_limit=last_n_sales,return_df=True
    )

    sales = new.append(old).sort_values('time').iloc[-last_n_sales:,:]
    sales["sale_min"] = sales["time"].dt.floor("min")
    ETH_USD =read_mongo("eth-usd",return_df=True, query_filter={'time':{'$in':sales.sale_min.to_list()}})
...
def preprocess_ApeGang(ape_ids=[], return_sales=True, update_events = True):
    ...
    if return_sales:
        ...
        sales_new = read_mongo(
            "ape-gang_sales", query_projection=mongo_projection, 
        ...
        latest_ETH_update = read_mongo("eth-usd",return_df=True, query_sort=[('time',-1)],query_limit=1,query_projection=['time'])
        ...
        rolling_average_USD = sales["sale-USD"].rolling(window=100,center=True).median()
        #first 50 sales are NA - so replace with expanding median
        rolling_average_USD.iloc[0:50] = sales["sale-USD"].iloc[0:50].expanding().median()
        #last 50 sales will also be NA, as we used centre = TRUE
        #so recalculate with centre = FALSE
        rolling_average_USD.iloc[-49:] = sales["sale-USD"].iloc[-200:].rolling(100, center=False).median().iloc[-49:]
...
def train_ApeGangML(
    ...
    rf = GridSearchCV(rf_reg,  param_grid={'n_estimators': [n_trees, 100], 
...
def update_ApeGang_pred_price():
    ...
    price_diff_df = read_mongo("ape-gang-USD-value",query_projection=['asset_id','pred_USD_price_diff','price_rank','rarity_rank'],
                                return_df=True)
